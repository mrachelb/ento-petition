...
var Bug = {
    ...
    animate: function(t) {
        ...
        if (--this.edge_test_counter <= 0 && this.bug_near_window_edge()) {
            ...
            if (
                Math.abs(this.directions[this.near_edge] - this.angle_deg) > 15
            ) {
                var angle1 = this.directions[this.near_edge] - this.angle_deg;
                var angle2 =
                    360 - this.angle_deg + this.directions[this.near_edge];
        ...
        var dx =
            Math.cos(this.angle_rad) * this.options.walkSpeed * (delta / 100);
        var dy =
            -Math.sin(this.angle_rad) * this.options.walkSpeed * (delta / 100);
    ...
    dropping: function(t, startPos, finalPos, rotationRate, deathType) {
        ...
        if (newPos >= finalPos) {
            ...
            this.bug.style.bottom =
                Math.ceil(rotationOffset - zoomOffset) + "px"; // because its rotated and zoomed.
